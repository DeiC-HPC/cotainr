...
3. Create a new branch and create new release notes in the format `YYYY.0M.MICRO.md` in the folder `doc/release_notes` based on the template `doc/release_notes/release_note.md.template` and fill in the release notes.
...
5. Tag the main branch locally using :code:`git tag YYYY.0M.MICRO` and push the tag to the GitHub repository :code:`git push origin tag YYYY.0M.MICRO`. This triggers the :ref:`CD on release <continuous_integration>` GitHub action.
